...
OS_DEBUG("OS_FreeRTOS_TimeBaseTaskEntryPoint callback was terminated.\n");
    vTaskSuspend( NULL );
    while (1)
    {
        taskYIELD();
...
int32 OS_TimeBaseCreate_Impl(const OS_object_token_t *token)
{
    ...
    // @TODO Reimplement timebase as a pure OSAL task with OSAL semaphores
    ...
    if (return_code == OS_SUCCESS)
    {
        ...
        if ( xTaskCreate(
                    ...
                    ( OS_TIMEBASE_TASK_STACK_SIZE / sizeof(StackType_t) ),  /* task stack size to be dynamically allocated */
                    timebase_id_variant.opaque_arg,                         /* pvParameters */
                    OS_FreeRTOS_MapOsalPriority(OS_TIMEBASE_TASK_PRIORITY),
        ...
        OS_BSP_GenericFreeRtosGlobal.timebase_task_handle = local->handler_task;
